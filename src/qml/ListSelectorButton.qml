...
ValueButton {
    ...
    function _updateValueText() {
        if ((model) && (model.count)) {
            valueText = model.data(Math.max(0, model.match(0, SelectionModel.ValueRole, value)),
            SelectionModel.NameRole);
        }
    }
    ...
    Component {
        ...
        ListPickSelector {
            ...
            onSelected: {
                root._ready = false;
                root.value = model.data(currentIndex, SelectionModel.ValueRole);
                root._ready = true;
                root.selected();
            }
            
            onStatusChanged: if (status == DialogStatus.Opening)
                             currentIndex = Math.max(0, model.match(0, SelectionModel.ValueRole, root.value));
